...
async function getAppleMusicTracks(
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `[Apple Music Scraper] Failed to fetch playlist page: status ${response.status}`,
      );
      return [];
    }
    ...
    if (serverDataText) {
      ...
      if (data && Array.isArray(data) && data.length > 0) {
        ...
        const trackSection = sections.find(
          (s: { itemKind?: string; id?: string }) =>
            s.itemKind === "trackLockup" || s.id?.startsWith("track-list"),
        );
        if (trackSection && Array.isArray(trackSection.items)) {
          return trackSection.items
            .map((item: { title: string; artistName?: string }) => ({
              title: item.title,
    ...
    if (jsonLd) {
      ...
      for (const item of items) {
        const trackProp =
          item.track || item.about?.track || item.mainEntity?.track;
        if (trackProp) {
          const trackArray = Array.isArray(trackProp)
            ? trackProp
            : [trackProp];
          ...
          const trackArray = Array.isArray(item.track)
            ? item.track
            : [item.track];
          for (const t of trackArray) {
            tracks.push({
    ...
    // 3. Dom fallback
    const domTracks: { title: string; artist: string }[] = [];
    $(".track-list-item").each((_i, el) => {
...
// ----------------------------------------------------------------
export async function fetchAppleTracks({
  id,
  url,
}: {
  type?: string;
