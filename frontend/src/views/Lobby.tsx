...
export default function Lobby() {
    const { roomCode, players, socket, beReady, musicAmount, setMusicAmount, time, setTime, playlistUrl, setPlaylistUrl, launchGame } = useGame();
    ...
    return (
        <div className="flex flex-col items-center gap-8 my-16">
                    ...
                    <div className="flex flex-col gap-6">
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="flex items-center justify-center gap-4">
                                {isHost &&
                                    <button className={`rounded-full bg-(--white) aspect-square text-(--background) h-8 w-8 ${musicAmount === 1 ? "opacity-50 cursor-not-allowed" : ""}`} onClick={() => { handleMusicAmount("down") }}>-</button>
                                }
                                <p>{musicAmount}</p>
                                {isHost &&
                                    <button className={`rounded-full bg-(--white) aspect-square text-(--background) h-8 w-8 ${musicAmount === 20 ? "opacity-50 cursor-not-allowed" : ""}`} onClick={() => { handleMusicAmount("up") }}>+</button>
                                }
                            </div>
                        </div>
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="flex items-center justify-center gap-4">
                                {isHost &&
                                    <button className={`rounded-full bg-(--white) aspect-square text-(--background) h-8 w-8 ${time === 5 ? "opacity-50 cursor-not-allowed" : ""}`} onClick={() => { handleTime("down") }}>-</button>
                                }
                                <p>{time}</p>
                                {isHost &&
                                    <button className={`rounded-full bg-(--white) aspect-square text-(--background) h-8 w-8 ${time === 30 ? "opacity-50 cursor-not-allowed" : ""}`} onClick={() => { handleTime("up") }}>+</button>
                                }
                            </div>
                        </div>
                    </div>
                ...
                <Section>
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <input type="text" className="h-8 w-full px-4 rounded-lg text-base bg-(--white) text-(--background)" value={playlistUrl} onChange={(e) => setPlaylistUrl(e.target.value)} />
                        </div>
                    </div>
                </Section>
            </div>
            ...
            <div className="w-full flex justify-between gap-4">
                ...
                {isHost &&
                    <button className="rounded-full px-8 py-2 bg-(--accent) text-(--white) flex-1" onClick={() => { launchGame() }}>Lancer</button>
                }
                {!isHost &&
